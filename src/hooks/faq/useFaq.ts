...
export function useGetFaqs(
  ...
  options?: Parameters<typeof useCoreQuery<TFetchFaqsResponse>>[2],
) {
  ...
  return useCoreQuery<TFetchFaqsResponse>(
    faqKeys.list(stable),
    () => getFaqs(stable),
    {
...
export function useSearchFaqs(
  params: TFaqSearchParams,
  options?: Parameters<typeof useCoreQuery<TFetchFaqsResponse>>[2],
) {
  const stable = useMemo(
    () => ({ ...params, keyword: params.keyword.trim() }),
    [params.keyword, params.category, params.page, params.size],
  );
  return useCoreQuery<TFetchFaqsResponse>(
    faqKeys.search(stable),
    () => searchFaqs(stable),
    {
