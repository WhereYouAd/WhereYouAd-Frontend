...
export const METRIC_LOCALE = "ko-KR" as const;
...
export function formatCurrencyRounded(v: number): string {
  return `₩${formatNumber(Math.round(v))}`;
}

/* KPI·통합 대시보드 트렌드 — 소수 2자리 */
export const formatPercentDelta = (v: number) => `${Math.abs(v).toFixed(2)}%`;
...
export interface IMetricMetaWithDelta extends IMetricMeta {
  formatDelta: (v: number) => string;
}

export interface IKpiMetricMeta extends IMetricMetaWithDelta {
  kpiLabel: string;
}

export interface IConversionMetricMeta extends IKpiMetricMeta {
  formatCard: (v: number) => string;
  formatCompact: (v: number) => string;
  formatDeltaCompact: (v: number) => string;
}

export interface IRoasMetricMeta extends IMetricMetaWithDelta {
  formatGrouped: (v: number) => string;
  formatTableTotal: (v: number) => string;
  formatTableRow: (v: number) => string;
}

export interface ICtrMetricMeta extends IMetricMetaWithDelta {
  formatCompact: (v: number) => string;
}

export interface IClicksMetricMeta extends IMetricMetaWithDelta {
...
export type TMetricKey = keyof typeof METRIC_REGISTRY;

/** KPI StatCard에 사용되는 Registry 키 (formatDelta 보유) */
export type TKpiMetricKey = "clicks" | "impressions" | "conversion" | "roas";

export function getMetric<K extends TMetricKey>(
  key: K,
): (typeof METRIC_REGISTRY)[K] {
...
/* API 응답 필드 → Registry 키 */
export const METRIC_API_TO_REGISTRY: Record<TMetricApiField, TMetricKey> = {
  clicks: "clicks",
  clickChangeRate: "clicks",
  impressions: "impressions",
  impressionChangeRate: "impressions",
  conversion: "conversion",
  cvrChangeRate: "conversion",
  ROAS: "roas",
  ROASChangeRate: "roas",
};

export interface IOverviewKpiBinding {
